...
def main():
    ...
    if params.debug:
        logger = init_logger('_log/03_make_sub.log', level=10)
    else:
    ...
    for i, target_token in enumerate(tqdm(target_tokens)):
        ...
        for _, target_sub in target_subs.iterrows():
            ...
            pred_box = Box(
                [x, y, z],
                [width, length, height],
                Quaternion(math.cos(rotate / 2), 0, 0, math.sin(rotate / 2))
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_sub_pre',
                        default=Path('_submission_pre/submit_model_th0.6_gth0.6_ov0.1.csv'), type=Path)
